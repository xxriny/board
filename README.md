...
애플리케이션 실행 후 `curl`과 `jq`를 사용하는 seed 스크립트로 테스트 데이터를 생성한다. 인자를 생략하면 게시글 10개와 게시글당 댓글 3개를 생성한다.
...
다른 주소, 비밀번호 또는 게시글별 최대 조회수를 사용할 때는 환경변수로 지정한다. 조회수는 기본적으로 게시글마다 0~100 사이의 랜덤값이며, seed 스크립트가 상세 조회 API를 해당 횟수만큼 호출해 실제 조회수 증가 로직을 사용한다.

```bash
BASE_URL=http://localhost:8081 \
SEED_PASSWORD=5678 \
MAX_VIEW_COUNT=500 \
./scripts/seed.sh 10 3
```

스크립트는 API를 호출하므로 비밀번호 해시와 `comment_count` 갱신을 포함한 실제 애플리케이션 흐름을 따른다. 기존 데이터는 삭제하지 않는다.
